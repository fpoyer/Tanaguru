...
public class UserManagementControllerTest extends TestCase {
    ...
    public void testDisplayAdminPageWithDeletedUserMessage() {
        ...
        assertEquals(TgolKeyStore.ADMIN_VIEW_NAME, result);
        assertTrue(model.asMap()
                .containsKey(TgolKeyStore.DELETED_USER_NAME_KEY));
    ...
    public void testEditUserAdminPage() {
        ...
        userId = "4";
        String result = instance.displayEditUserAdminPage(userId, request,
                response, model);
        assertEquals(TgolKeyStore.EDIT_USER_VIEW_NAME, result);
        CreateUserCommand cuc = ((CreateUserCommand) model.asMap().get(
                TgolKeyStore.CREATE_USER_COMMAND_KEY));
        assertNotNull(cuc);
        ...
        assertEquals(Long.valueOf(4),
                request.getSession().getAttribute(TgolKeyStore.USER_ID_KEY));

        userId = "5";
        result = instance.displayEditUserAdminPage(userId, request, response,
                model);
        assertEquals(TgolKeyStore.EDIT_USER_VIEW_NAME, result);
        cuc = ((CreateUserCommand) model.asMap().get(
                TgolKeyStore.CREATE_USER_COMMAND_KEY));
        assertNotNull(cuc);
    ...
    public void testSubmitEditUserForm() throws Exception {
        ...
        CreateUserCommand createUserCommand = CreateUserCommandFactory
                .getInstance().getNewCreateUserCommand();
        ...
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.getSession().setAttribute(TgolKeyStore.USER_ID_KEY,
                Long.valueOf(5));
        BindingResult bindingResult = new BeanPropertyBindingResult(
                createUserCommand, "createUserCommand");
        Model model = new ExtendedModelMap();
        String result = instance.submitEditUserForm(createUserCommand,
                bindingResult, request, model);
        ...
        assertEquals("user@test.com",
                model.asMap().get(TgolKeyStore.UPDATED_USER_NAME_KEY));
        assertTrue(((List<User>) model.asMap().get(TgolKeyStore.USER_LIST_KEY))
                .isEmpty());
    }
    ...
    public void testSubmitEditUserChangeToExistingUserForm() throws Exception {
        ...
        assertEquals("sign-up.existingAccountWithEmail", bindingResult
                .getFieldErrors("email").get(0).getCode());
    }
    ...
    public void testAddUserAdminPage() {
        ...
        String result = instance.displayAddUserAdminPage(request, response,
                model);
    ...
    public void testSubmitAddUserForm() throws Exception {
        ...
        BindingResult bindingResult = new BeanPropertyBindingResult(
                createUserCommand, "createUserCommand");
        Model model = new ExtendedModelMap();
        String result = instance.submitAddUserForm(createUserCommand,
                bindingResult, model);
        ...
        assertTrue(bindingResult.getFieldErrors().isEmpty());
        assertEquals(2, model.asMap().size());
        assertEquals("newUser@test.com",
                model.asMap().get(TgolKeyStore.ADDED_USER_NAME_KEY));
        assertTrue(((List<User>) model.asMap().get(TgolKeyStore.USER_LIST_KEY))
                .isEmpty());
    }
    ...
    public void testSubmitAddUserFormWithErrors() throws Exception {
        ...
        assertNotNull(bindingResult.getFieldErrors("email"));
        assertEquals(1, bindingResult.getFieldErrors("email").size());
        assertEquals("sign-up.missingEmail",
                bindingResult.getFieldErrors("email").get(0).getCode());
        assertNotNull(bindingResult.getFieldErrors("password"));
        assertEquals(1, bindingResult.getFieldErrors("password").size());
        assertEquals("sign-up.missingPassword",
                bindingResult.getFieldErrors("password").get(0).getCode());
        ...
        assertEquals("sign-up.mandatoryField",
                bindingResult.getFieldErrors("generalErrorMsg").get(0)
                        .getCode());

        // First the form is sent back with passwords that don't match
        createUserCommand = CreateUserCommandFactory.getInstance()
                .getNewCreateUserCommand();
        createUserCommand.setEmail("user@test.com");
        createUserCommand.setPassword("password");
        createUserCommand.setConfirmPassword("confirmation");
        bindingResult = new BeanPropertyBindingResult(createUserCommand,
                "createUserCommand");
        model = new ExtendedModelMap();
        result = instance.submitAddUserForm(createUserCommand, bindingResult,
                model);
        ...
        assertNotNull(bindingResult.getFieldErrors("password"));
        assertEquals(1, bindingResult.getFieldErrors("password").size());
        assertEquals("sign-up.passwordNotIdentical", bindingResult
                .getFieldErrors("password").get(0).getCode());
        assertNotNull(bindingResult.getFieldErrors("generalErrorMsg"));
        assertEquals(1, bindingResult.getFieldErrors("generalErrorMsg").size());
        assertEquals("sign-up.mandatoryField",
                bindingResult.getFieldErrors("generalErrorMsg").get(0)
        ...
        // Then the url is not conform but in this context it is not tested, so
        // ignored
        ...
        assertNotNull(bindingResult.getFieldErrors("email"));
        assertEquals(1, bindingResult.getFieldErrors("email").size());
        assertEquals("sign-up.invalidEmail",
                bindingResult.getFieldErrors("email").get(0).getCode());
        assertNotNull(bindingResult.getFieldErrors("password"));
        assertEquals(1, bindingResult.getFieldErrors("password").size());
        assertEquals("sign-up.invalidPassword",
                bindingResult.getFieldErrors("password").get(0).getCode());
        assertNotNull(bindingResult.getFieldErrors("generalErrorMsg"));
        assertEquals(1, bindingResult.getFieldErrors("generalErrorMsg").size());
        assertEquals("sign-up.mandatoryField",
    ...
    public void testDeleteUserPage() {
        ...
        idToRemove = "4";
        String result = instance.displayDeleteUserPage(idToRemove, request,
                response, model);
        assertEquals(TgolKeyStore.ACCESS_DENIED_VIEW_NAME, result);

        idToRemove = "6";
        result = instance.displayDeleteUserPage(idToRemove, request, response,
                model);
        assertEquals(TgolKeyStore.ACCESS_DENIED_VIEW_NAME, result);
        ...
        assertEquals("user@test.com",
                model.asMap().get(TgolKeyStore.USER_NAME_TO_DELETE_KEY));
        assertEquals(
                Long.valueOf(5),
    ...
    public void testDeleteUserConfirmation() {
        ...
        request.getSession().setAttribute(TgolKeyStore.USER_ID_TO_DELETE_KEY,
                idToRemove);
        String result = instance.displayDeleteUserConfirmation(request,
                response, model);
        assertEquals(TgolKeyStore.ACCESS_DENIED_VIEW_NAME, result);

        request.getSession().setAttribute(TgolKeyStore.USER_ID_TO_DELETE_KEY,
                Long.valueOf(6));
        result = instance.displayDeleteUserConfirmation(request, response,
                model);
        assertEquals(TgolKeyStore.ACCESS_DENIED_VIEW_NAME, result);
    ...
    private void setUpMockAuthenticationContext() {
        ...
        TgolUserDetails tud = new TgolUserDetails("test1@test.com", "", true,
                false, true, true, gac, mockAdminUser);
        ...
        expect(mockAuthentication.getName()).andReturn("admin@test.com")
                .anyTimes();
    ...
    private void setUpMockContractDataService() {
        ...
        Collection<Contract> contractSet = Collections.emptyList();
        expect(mockContractDataService.findAll()).andReturn(
                contractSet).anyTimes();
        replay(mockContractDataService);
    }

    private void setUpMockUserDataService(boolean hasDeleteCall,
            boolean hasCreateCall, boolean activatedAndAdmin,
            boolean hasUpdateUserCall, boolean hasUpdateAdminCall) {
        // initialise the context with the user identified by the email
        // "test1@test.com" seen as authenticated
        ...
        expect(mockUserDataService.findAll()).andReturn(userCollection)
                .anyTimes();

        expect(mockUserDataService.read(Long.valueOf(4))).andReturn(
                mockAdminUser).anyTimes();
        expect(mockUserDataService.getUserFromEmail("admin@test.com"))
                .andReturn(mockAdminUser).anyTimes();

        expect(mockUserDataService.read(Long.valueOf(5))).andReturn(mockUser)
                .anyTimes();
        expect(mockUserDataService.getUserFromEmail("user@test.com"))
                .andReturn(mockUser).anyTimes();

        expect(mockUserDataService.read(Long.valueOf(6))).andReturn(null)
                .anyTimes();
        expect(mockUserDataService.getUserFromEmail("null")).andReturn(null)
                .anyTimes();

        expect(mockUserDataService.getUserFromEmail("newUser@test.com"))
        ...
        if (hasCreateCall) {
            ...
            expect(mockUserDataService.saveOrUpdate(mockNewUser)).andReturn(
                    mockNewUser).once();
            expect(mockNewUser.getEmail1()).andReturn("newUser@test.com")
                    .once();
        }
        if (hasUpdateUserCall) {
            ...
            expectLastCall().once();
            expect(mockUserDataService.saveOrUpdate(mockUser)).andReturn(
                    mockUser).once();
        ...
        if (hasUpdateAdminCall) {
            ...
            expectLastCall().once();
            expect(mockUserDataService.saveOrUpdate(mockAdminUser)).andReturn(
                    mockAdminUser).once();
            expect(mockAdminUser.getEmail1()).andReturn("admin@test.com")
                    .once();
        }
        expect(mockAdminUser.getEmail1()).andReturn("admin@test.com")
                .anyTimes();
        expect(mockAdminUser.getWebUrl1()).andReturn("http://www.admin.com")
                .anyTimes();
        expect(mockAdminUser.getFirstName()).andReturn("firstNameAdmin")
                .anyTimes();
        expect(mockAdminUser.getName()).andReturn("nameAdmin").anyTimes();
        expect(mockAdminUser.getPhoneNumber()).andReturn("0123456789")
                .anyTimes();
        ...
        expect(mockUser.getWebUrl1()).andReturn("http://www.user.com")
                .anyTimes();
        ...
        // expect(mockUser.getContractSet()).andReturn(new
        // ArrayList<Contract>()).anyTimes();
    ...
    private void setUpMockRoleDataService() {
        ...
        expect(mockRoleDataService.read(Long.valueOf(2))).andReturn(
                mockUserRole).anyTimes();
        expect(mockRoleDataService.read(Long.valueOf(3))).andReturn(
                mockAdminRole).anyTimes();
