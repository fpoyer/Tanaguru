...
import org.opens.tgol.entity.user.UserFromLdap;
...
public abstract class AbstractController {
	...
	@Autowired
	public final void setContractDataService(
	 ...
	 * session
	 * 
	 * @return true if a user is authenticated in the current session, false
	 *         otherwise.
	 */
	protected boolean isAuthenticated() {
		if (!SecurityContextHolder.getContext().getAuthentication()
				.isAuthenticated()) {
			return false;
			// By default, an unauthenticated user is authenticated by the
			// application as
			// an anonymousUser
		} else if (SecurityContextHolder.getContext().getAuthentication()
				.getName().equalsIgnoreCase(ANONYMOUS_USER)) {
			return false;
		} else {
			return true;
		}
	...
	/**
	 * This method determines whether the authenticated user of the current
	 * session is a guest
	 * 
	 * @return true if the the authenticated user of the current session is a
	...
	protected boolean isGuestUser() {
		...
		if (authorities.size() == 1
				&& authorities.iterator().next().getAuthority()
						.equalsIgnoreCase(TgolKeyStore.ROLE_GUEST_KEY)) {
			return true;
		}
		return false;
	}
	...
	protected boolean isAdminUser() {
		...
		if (authorities != null
				&& authorities.size() == 1
				&& authorities.iterator().next().getAuthority()
						.equalsIgnoreCase(TgolKeyStore.ROLE_ADMIN_KEY)) {
			return true;
		}
		return false;
	}
	...
	protected User getCurrentUser() {
		boolean authenticationIsNull = (SecurityContextHolder.getContext()
				.getAuthentication() == null);
		if (!authenticationIsNull) {

			Object principal = SecurityContextHolder.getContext()
					.getAuthentication().getPrincipal();
			if (principal instanceof TgolUserDetails) {
				return ((TgolUserDetails) principal).getUser();
			} else if (principal instanceof LdapUserDetails) {
				LdapUserDetails ldapUserDetails = ((LdapUserDetails) principal);
				return new UserFromLdap(ldapUserDetails, contractDataService.findAll());
			}
		}
	...
	protected void updateCurrentUser(User user) {
		TgolUserDetails userDetails = ((TgolUserDetails) SecurityContextHolder
	...
	protected boolean isContractExpired(Contract contract) {
		try {
			return (Calendar.getInstance().getTime().after(contract
	...
	/**
	 * To deal with contract expiration this method is defined here and
	 * accessible from extended classes when needed. The related jsp uses the
	 * IS_CONTRACT_EXPIRED_KEY to enable or not the launch actions.
	 * 
	 * @param contract
	 * @param model
	 * @return
